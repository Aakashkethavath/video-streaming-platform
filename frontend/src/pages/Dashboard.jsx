...
const Dashboard = () => {
  ...
  const [filterSize, setFilterSize] = useState("all"); // <--- NEW: Size Filter
  ...
  const filteredAdminVideos = Array.isArray(adminVideos) ? adminVideos.filter(v => {
    ...
    if (filterSize === "small") matchesSize = sizeMB < 10;       // Less than 10MB
    if (filterSize === "medium") matchesSize = sizeMB >= 10 && sizeMB < 50; // 10-50MB
    if (filterSize === "large") matchesSize = sizeMB >= 50;      // More than 50MB

    return matchesEditor && matchesDate && matchesSize;
  }) : [];
          ...
          <div className="card" style={{ maxWidth: '100%', marginBottom: '2rem', border: '1px solid #e5e7eb' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '10px' }}>
              ...
              <div style={{ display: 'flex', gap: '10px' }}>
                ...
                {(filterEditor || filterDate || filterSize !== "all") && (
                  <button 
                    onClick={() => { setFilterEditor(""); setFilterDate(""); setFilterSize("all"); }}
